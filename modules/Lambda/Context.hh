...
#include "Types.hh"
#include "Config.hh"          // Config::Watch
#include "Monitor.hh"         // Monitor::AsyncLogger
#include "Record/Writer.hh"   // Record::Writer

// ── Lambda handler context bundles ───────────────────────────────────────────
// Each analysis handler receives a *context* struct instead of many individual
...
//
// Two structs, not one: the output target differs between the histogram
// handlers (AnalysisContext → RootArray) and the data-generation handler
...
namespace Lambda {
    ...
    struct AnalysisContext {
        ...
        Record::Writer&       writer;      // fill mutex + checkpoint/shutdown
    };
    ...
    struct GenerationContext {
        ...
        Config::Watch&        logging;     // live event counters
        Monitor::AsyncLogger& asyncLogger; // progress publisher
        Record::Writer&       writer;      // available for parity; checkpoint if needed
    };
